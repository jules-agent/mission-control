...
-- Partial unique index to ensure only one base identity per user
CREATE UNIQUE INDEX unique_base_per_user ON identities(user_id, is_base) WHERE is_base = true;
